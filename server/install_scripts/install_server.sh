...
#   --api-port PORT     ss-api HTTP portu (varsayilan: 8087)
#   --manager-port PORT ss-manager UDP portu (varsayilan: 6001)
...
Type=simple
User=root
ExecStart=/usr/local/bin/ss-manager -u --manager-address 127.0.0.1:${MANAGER_PORT} --workdir ${LIBEV_WORKDIR} -s 0.0.0.0 -m chacha20-ietf-poly1305
Restart=on-failure
RestartSec=3
...
ExecStart=/usr/bin/python3 ${LIBEV_SS_API_DIR}/ss_api.py --host 127.0.0.1 --port ${API_PORT} --manager-address 127.0.0.1:${MANAGER_PORT} --server-ip ${PUBLIC_HOSTNAME} --api-secret ${LIBEV_API_SECRET} --port-store ${LIBEV_WORKDIR}/ports.json
Restart=on-failure
RestartSec=3
...
function write_access_config() {
    ...
    local api_url="http://127.0.0.1:${API_PORT}/${LIBEV_API_SECRET}"
    cat > "${ACCESS_CONFIG}" <<EOF
apiUrl:${api_url}
serverIp:${PUBLIC_HOSTNAME}
managerAddress:127.0.0.1:${MANAGER_PORT}
type:libev
...
Bot / yonetim API JSON (config outline_apis icine yapistirin):
...
function main() {
    ...
    MANAGER_PORT="${FLAGS_MANAGER_PORT}"
    if (( MANAGER_PORT == 0 )); then
        MANAGER_PORT=6001
    fi
    readonly MANAGER_PORT
